...
/// Signature used by [SwiftNestedScrollView] for building its header.
///
...
typedef SwiftNestedScrollViewHeaderSliversBuilder =
...
/// to expand.
///
/// [SwiftNestedScrollView] solves this problem by providing custom
...
///
/// {@tool dartpad}
/// This example shows a [SwiftNestedScrollView] whose header is the combination of a
/// [TabBar] in a [SliverAppBar] and whose body is a [TabBarView]. It uses a
/// [SwiftSliverOverlapAbsorber]/[SwiftSliverOverlapInjector] pair to make the inner lists
...
/// {@end-tool}
///
/// ## [SliverAppBar]s with [SwiftNestedScrollView]s
///
/// Using a [SliverAppBar] in the outer scroll view, or [headerSliverBuilder],
/// of a [SwiftNestedScrollView] may require special configurations in order to work
/// as it would if the outer and inner were one single scroll view, like a
/// [CustomScrollView].
///
/// ### Pinned [SliverAppBar]s
///
/// A pinned [SliverAppBar] works in a [SwiftNestedScrollView] exactly as it would in
...
///
/// This works naturally in a [SwiftNestedScrollView], as the pinned [SliverAppBar]
...
///
/// In order to float the outer, use [SwiftNestedScrollView.floatHeaderSlivers]. When
...
///
/// {@tool dartpad}
/// This simple example shows a [SwiftNestedScrollView] whose header contains a
...
///
/// It is possible with a [SwiftNestedScrollView] to perform just the snapping
/// animation without floating the app bar in and out. By not using the
/// [SwiftNestedScrollView.floatHeaderSlivers], the app bar will snap in and out
/// without floating.
///
/// The [SliverAppBar.snap] animation should be used in conjunction with the
/// [SwiftSliverOverlapAbsorber] and  [SwiftSliverOverlapInjector] widgets when
/// implemented in a [SwiftNestedScrollView]. These widgets take any overlapping
/// behavior of the [SliverAppBar] in the header and redirect it to the
/// [SwiftSliverOverlapInjector] in the body. If it is missing, then it is possible
...
/// e.g [SwiftNestedScrollView.floatHeaderSlivers], the [SliverAppBar] will animate
/// in and out without floating. The [SwiftSliverOverlapAbsorber] and
/// [SwiftSliverOverlapInjector] maintain the proper alignment between the two
...
/// Currently, [SwiftNestedScrollView] does not support simultaneously floating and
...
/// Currently, [SwiftNestedScrollView] does not support stretching the outer
...
///  * [SwiftSliverOverlapAbsorber], a sliver that wraps another, forcing its layout
///    extent to be treated as overlap.
///  * [SwiftSliverOverlapInjector], a sliver that has a sliver geometry based on
///    the values stored in a [SwiftSliverOverlapAbsorberHandle].
class SwiftNestedScrollView extends StatefulWidget {
  /// Creates a nested scroll view.
  ///
  /// The [reverse], [headerSliverBuilder], and [body] arguments must not be
  /// null.
  const SwiftNestedScrollView({
    super.key,
    this.controller,
  ...
  /// inner scroll view is not directly configured by the [SwiftNestedScrollView],
  ...
  /// for flexible configurations of the SwiftNestedScrollView.
  ...
  /// is not directly configured by the [SwiftNestedScrollView]. For both to scroll
  ...
  /// SwiftNestedScrollView.
  ///
  /// The [ScrollPhysics] also determine whether or not the [SwiftNestedScrollView]
  ...
  final SwiftNestedScrollViewHeaderSliversBuilder headerSliverBuilder;

  /// The widget to show inside the [SwiftNestedScrollView].
  ///
  /// Typically this will be [TabBarView].
  ///
  /// The [body] is built in a context that provides a [PrimaryScrollController]
  /// that interacts with the [SwiftNestedScrollView]'s scroll controller. Any
  /// [ListView] or other [Scrollable]-based widget inside the [body] that is
  /// intended to scroll with the [SwiftNestedScrollView] should therefore not be
  /// given an explicit [ScrollController], instead allowing it to default to
  /// the [PrimaryScrollController] provided by the [SwiftNestedScrollView].
  final Widget body;

  /// A wrapper for the list of slivers returned by [headerSliverBuilder].
  ///
  /// This allows wrapping the header slivers in widgets like [SliverMainAxisGroup],
  /// [SliverOverlapAbsorber], or any other sliver widget.
  final Widget Function(BuildContext context, List<Widget> headerSlivers)?
  headerSliversWrapper;

  /// {@macro flutter.widgets.scrollable.dragStartBehavior}
  final DragStartBehavior dragStartBehavior;

  /// Whether or not the [SwiftNestedScrollView]'s coordinator should prioritize the
  ...
  /// SwiftNestedScrollView cannot assume the configuration of the outer and inner
  ...
  /// Returns the [SwiftSliverOverlapAbsorberHandle] of the nearest ancestor
  /// [SwiftNestedScrollView].
  ///
  /// This is necessary to configure the [SwiftSliverOverlapAbsorber] and
  /// [SwiftSliverOverlapInjector] widgets.
  ///
  /// For sample code showing how to use this method, see the [SwiftNestedScrollView]
  /// documentation.
  static SwiftSliverOverlapAbsorberHandle sliverOverlapAbsorberHandleFor(
    BuildContext context,
  ) {
    final _InheritedSwiftNestedScrollView? target = context
        .dependOnInheritedWidgetOfExactType<_InheritedSwiftNestedScrollView>();
    assert(
      target != null,
      'SwiftNestedScrollView.sliverOverlapAbsorberHandleFor must be called with a context that contains a SwiftNestedScrollView.',
    );
  ...
  List<Widget> _buildSlivers(
    ...
    final List<Widget> headers = headerSliverBuilder(context, bodyIsScrolled);
    return <Widget>[
      if (headerSliversWrapper != null)
        headerSliversWrapper!(context, headers)
      else
        ...headers,
      SliverFillRemaining(
        ...
        child: PrimaryScrollController(
          ...
          // `SwiftNestedScrollView.scrollDirection` for the PrimaryScrollController,
  ...
  @override
  SwiftNestedScrollViewState createState() => SwiftNestedScrollViewState();
}

/// The [State] for a [SwiftNestedScrollView].
///
/// The [ScrollController]s, [innerController] and [outerController], of the
/// [SwiftNestedScrollView]'s children may be accessed through its state. This is
/// useful for obtaining respective scroll positions in the [SwiftNestedScrollView].
///
/// If you want to access the inner or outer scroll controller of a
/// [SwiftNestedScrollView], you can get its [SwiftNestedScrollViewState] by supplying a
/// `GlobalKey<SwiftNestedScrollViewState>` to the [SwiftNestedScrollView.key] parameter).
///
/// {@tool dartpad}
/// [SwiftNestedScrollViewState] can be obtained using a [GlobalKey].
...
/// {@end-tool}
class SwiftNestedScrollViewState extends State<SwiftNestedScrollView> {
  final SwiftSliverOverlapAbsorberHandle _absorberHandle =
      SwiftSliverOverlapAbsorberHandle();

  /// The [ScrollController] provided to the [ScrollView] in
  /// [SwiftNestedScrollView.body].
  ///
  ...
  ///    sliver(s) contained in [SwiftNestedScrollView.headerSliverBuilder].
  ScrollController get innerController => _coordinator!._innerController;

  /// The [ScrollController] provided to the [ScrollView] in
  /// [SwiftNestedScrollView.headerSliverBuilder].
  ///
  /// This is equivalent to [SwiftNestedScrollView.controller], if provided.
  ///
  ...
  ///    [ScrollView] contained in [SwiftNestedScrollView.body].
  ScrollController get outerController => _coordinator!._outerController;

  _SwiftNestedScrollCoordinator? _coordinator;

  @protected
  @override
  void initState() {
    super.initState();
    _coordinator = _SwiftNestedScrollCoordinator(
      this,
      widget.controller,
  ...
  @protected
  @override
  void didUpdateWidget(SwiftNestedScrollView oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    return _InheritedSwiftNestedScrollView(
      state: this,
      child: Builder(
        builder: (BuildContext context) {
          _lastHasScrolledBody = _coordinator!.hasScrolledBody;
          return _SwiftNestedScrollViewCustomScrollView(
...
class _SwiftNestedScrollViewCustomScrollView extends CustomScrollView {
  const _SwiftNestedScrollViewCustomScrollView({
  ...
  final SwiftSliverOverlapAbsorberHandle handle;
  ...
  Widget buildViewport(
    ...
    assert(!shrinkWrap);
    return SwiftNestedScrollViewViewport(
      axisDirection: axisDirection,
      offset: offset,
      slivers: slivers,
      handle: handle,
...
class _InheritedSwiftNestedScrollView extends InheritedWidget {
  const _InheritedSwiftNestedScrollView({
    required this.state,
    required super.child,
  });

  final SwiftNestedScrollViewState state;

  @override
  bool updateShouldNotify(_InheritedSwiftNestedScrollView old) =>
      state != old.state;
}

class _SwiftNestedScrollMetrics extends FixedScrollMetrics {
  _SwiftNestedScrollMetrics({
  ...
  @override
  _SwiftNestedScrollMetrics copyWith({
    ...
    return _SwiftNestedScrollMetrics(
      minScrollExtent:
          minScrollExtent ??
...
typedef _SwiftNestedScrollActivityGetter =
    ScrollActivity Function(_SwiftNestedScrollPosition position);

class _SwiftNestedScrollCoordinator
    implements ScrollActivityDelegate, ScrollHoldController {
  _SwiftNestedScrollCoordinator(
    this._state,
    this._parent,
    this._onHasScrolledBodyChanged,
    this._floatHeaderSlivers,
  ) {
    // assert(debugMaybeDispatchCreated('widgets', '_SwiftNestedScrollCoordinator', this));
    final double initialScrollOffset = _parent?.initialScrollOffset ?? 0.0;
    _outerController = _SwiftNestedScrollController(
      this,
      initialScrollOffset: initialScrollOffset,
      debugLabel: 'outer',
    );
    _innerController = _SwiftNestedScrollController(this, debugLabel: 'inner');
  }

  final SwiftNestedScrollViewState _state;
  ScrollController? _parent;
  final VoidCallback _onHasScrolledBodyChanged;
  final bool _floatHeaderSlivers;

  late _SwiftNestedScrollController _outerController;
  late _SwiftNestedScrollController _innerController;

  bool get outOfRange {
    return (_outerPosition?.outOfRange ?? false) ||
        _innerPositions.any(
          (_SwiftNestedScrollPosition position) => position.outOfRange,
        );
  }

  _SwiftNestedScrollPosition? get _outerPosition {
  ...
  Iterable<_SwiftNestedScrollPosition> get _innerPositions {
    return _innerController.nestedPositions;
  }

  bool get canScrollBody {
    final _SwiftNestedScrollPosition? outer = _outerPosition;
    if (outer == null) {
      return true;
    }
  ...
  bool get hasScrolledBody {
    for (final _SwiftNestedScrollPosition position in _innerPositions) {
      if (!position.hasContentDimensions || !position.hasPixels) {
        // It's possible that SwiftNestedScrollView built twice before layout phase
        // in the same frame. This can happen when the FocusManager schedules a microTask
        // that marks SwiftNestedScrollView dirty during the warm up frame.
  ...
  void beginActivity(
    ...
    _SwiftNestedScrollActivityGetter innerActivityGetter,
  ) {
  ...
  static IdleScrollActivity _createIdleScrollActivity(
    _SwiftNestedScrollPosition position,
  ) {
  ...
  ScrollActivity createOuterBallisticScrollActivity(double velocity) {
    ...
    // are heading towards.
    _SwiftNestedScrollPosition? innerPosition;
    if (velocity != 0.0) {
      for (final _SwiftNestedScrollPosition position in _innerPositions) {
    ...
    final _SwiftNestedScrollMetrics metrics = _getMetrics(
      innerPosition,
      velocity,
    );
  ...
  _SwiftNestedScrollMetrics _getMetrics(
    _SwiftNestedScrollPosition innerPosition,
    double velocity,
  ) {
  ...
  double unnestOffset(double value, _SwiftNestedScrollPosition source) {
  ...
  double nestOffset(double value, _SwiftNestedScrollPosition target) {
  ...
  void updateCanDrag() {
    ...
    for (final _SwiftNestedScrollPosition position in _innerPositions) {
      ...
      innerCanDrag =
          ...
          // the whole SwiftNestedScrollView, since it is possible to have different
  ...
  Future<void> animateTo(
    ...
    final DrivenScrollActivity outerActivity = _outerPosition!
    ...
    beginActivity(outerActivity, (_SwiftNestedScrollPosition position) {
      final DrivenScrollActivity innerActivity = position
          .createDrivenScrollActivity(
  ...
  void pointerScroll(double delta) {
    ...
    // the delta, the SwiftNestedScrollView's intention is to treat multiple
    ...
    if (_innerPositions.isEmpty) {
      ...
      for (final _SwiftNestedScrollPosition position in _innerPositions) {
        if (position.pixels < 0.0) {
          ...
          final double potentialOuterDelta = position
      ...
      if (outerDelta != 0.0) {
        final double innerDelta = _outerPosition!
            .applyClampedPointerSignalUpdate(outerDelta);
        if (innerDelta != 0.0) {
          for (final _SwiftNestedScrollPosition position in _innerPositions) {
      ...
      if (innerDelta != 0.0) {
        ...
        for (final _SwiftNestedScrollPosition position in _innerPositions) {
          final double overscroll = position.applyClampedPointerSignalUpdate(
            innerDelta,
          );
  ...
  void applyUserOffset(double delta) {
    ...
    if (_innerPositions.isEmpty) {
      ...
      for (final _SwiftNestedScrollPosition position in _innerPositions) {
        if (position.pixels < 0.0) {
          // This inner position is in overscroll.
          final double potentialOuterDelta = position.applyClampedDragUpdate(
            delta,
          );
          // In case there are multiple positions in varying states of
          // overscroll, the first to 'reach' the outer view above takes
      ...
      if (innerDelta != 0.0) {
        ...
        final List<_SwiftNestedScrollPosition> innerPositions = _innerPositions
            .toList();
  ...
  @override
  String toString() =>
      '${objectRuntimeType(this, '_SwiftNestedScrollCoordinator')}(outer=$_outerController; inner=$_innerController)';
}

class _SwiftNestedScrollController extends ScrollController {
  _SwiftNestedScrollController(
    this.coordinator, {
    super.initialScrollOffset,
    super.debugLabel,
  });

  final _SwiftNestedScrollCoordinator coordinator;
  ...
  ScrollPosition createScrollPosition(
    ...
    return _SwiftNestedScrollPosition(
  ...
  void attach(ScrollPosition position) {
    assert(position is _SwiftNestedScrollPosition);
  ...
  void detach(ScrollPosition position) {
    assert(position is _SwiftNestedScrollPosition);
    (position as _SwiftNestedScrollPosition).setParent(null);
  ...
  void _scheduleUpdateShadow() {
    ...
    }, debugLabel: 'SwiftNestedScrollController.updateShadow');
  }

  Iterable<_SwiftNestedScrollPosition> get nestedPositions {
    return positions.cast<_SwiftNestedScrollPosition>();
  }
}

// The _SwiftNestedScrollPosition is used by both the inner and outer viewports of a
// SwiftNestedScrollView. It tracks the offset to use for those viewports, and knows
// about the _SwiftNestedScrollCoordinator, so that when activities are triggered on
// this class, they can defer, or be influenced by, the coordinator.
class _SwiftNestedScrollPosition extends ScrollPosition
    implements ScrollActivityDelegate {
  _SwiftNestedScrollPosition({
  ...
  ScrollActivity createBallisticScrollActivity(
    ...
    _SwiftNestedScrollMetrics? metrics,
  }) {
    if (simulation == null) {
...
class _NestedInnerBallisticScrollActivity extends BallisticScrollActivity {
  ...
  @override
  _SwiftNestedScrollPosition get delegate =>
      super.delegate as _SwiftNestedScrollPosition;
...
class _NestedOuterBallisticScrollActivity extends BallisticScrollActivity {
  ...
  final _SwiftNestedScrollCoordinator coordinator;
  final _SwiftNestedScrollMetrics metrics;
...
/// Handle to provide to a [SwiftSliverOverlapAbsorber], a [SwiftSliverOverlapInjector],
/// and an [SwiftNestedScrollViewViewport], to shift overlap in a [SwiftNestedScrollView].
///
/// A particular [SwiftSliverOverlapAbsorberHandle] can only be assigned to a single
/// [SwiftSliverOverlapAbsorber] at a time. It can also be (and normally is) assigned
/// to one or more [SwiftSliverOverlapInjector]s, which must be later descendants of
/// the same [SwiftNestedScrollViewViewport] as the [SwiftSliverOverlapAbsorber]. The
/// [SwiftSliverOverlapAbsorber] must be a direct descendant of the
/// [SwiftNestedScrollViewViewport], taking part in the same sliver layout. (The
/// [SwiftSliverOverlapInjector] can be a descendant that takes part in a nested
/// scroll view's sliver layout.)
///
/// Whenever the [SwiftNestedScrollViewViewport] is marked dirty for layout, it will
/// cause its assigned [SwiftSliverOverlapAbsorberHandle] to fire notifications. It
/// is the responsibility of the [SwiftSliverOverlapInjector]s (and any other
/// clients) to mark themselves dirty when this happens, in case the geometry
/// subsequently changes during layout.
///
/// See also:
///
///  * [SwiftNestedScrollView], which uses a [SwiftNestedScrollViewViewport] and a
///    [SwiftSliverOverlapAbsorber] to align its children, and which shows sample
///    usage for this class.
class SwiftSliverOverlapAbsorberHandle extends ChangeNotifier {
  /// Creates a [SwiftSliverOverlapAbsorberHandle].
  SwiftSliverOverlapAbsorberHandle() {
  ...
  // Incremented when a RenderSwiftSliverOverlapAbsorber takes ownership of this
  ...
  /// [SwiftSliverOverlapAbsorber].
  ///
  /// This corresponds to the [SliverGeometry.layoutExtent] of the child of the
  /// [SwiftSliverOverlapAbsorber].
  ///
  /// This is updated during the layout of the [SwiftSliverOverlapAbsorber]. It
  /// should not change at any other time. No notifications are sent when it
  /// changes; clients (e.g. [SwiftSliverOverlapInjector]s) are responsible for
  /// marking themselves dirty whenever this object sends notifications, which
  /// happens any time the [SwiftSliverOverlapAbsorber] might subsequently change the
  ...
  void _setExtents(double? layoutValue, double? scrollValue) {
    assert(
      _writers == 1,
      'Multiple RenderSwiftSliverOverlapAbsorbers have been provided the same SwiftSliverOverlapAbsorberHandle.',
    );
  ...
  String toString() {
    ...
    return '${objectRuntimeType(this, 'SwiftSliverOverlapAbsorberHandle')}($layoutExtent$extra)';
  }
}
...
/// to the [SwiftSliverOverlapAbsorberHandle], which is typically passed to a
/// [SwiftSliverOverlapInjector].
///
/// See also:
///
///  * [SwiftNestedScrollView], whose documentation has sample code showing how to
///    use this widget.
class SwiftSliverOverlapAbsorber extends SingleChildRenderObjectWidget {
  /// Creates a sliver that absorbs overlap and reports it to a
  /// [SwiftSliverOverlapAbsorberHandle].
  const SwiftSliverOverlapAbsorber({
    super.key,
    required this.handle,
    Widget? sliver,
  }) : super(child: sliver);
  ...
  ///
  /// A particular [SwiftSliverOverlapAbsorberHandle] can only be assigned to a
  /// single [SwiftSliverOverlapAbsorber] at a time.
  final SwiftSliverOverlapAbsorberHandle handle;

  @override
  RenderSwiftSliverOverlapAbsorber createRenderObject(BuildContext context) {
    return RenderSwiftSliverOverlapAbsorber(handle: handle);
  }

  @override
  void updateRenderObject(
    BuildContext context,
    RenderSwiftSliverOverlapAbsorber renderObject,
  ) {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<SwiftSliverOverlapAbsorberHandle>('handle', handle),
    );
  }
}
...
/// overlap reported by this widget, called the _absorbed overlap_, is reported
/// to the [SwiftSliverOverlapAbsorberHandle], which is typically passed to a
/// [RenderSwiftSliverOverlapInjector].
class RenderSwiftSliverOverlapAbsorber extends RenderSliver
    with RenderObjectWithChildMixin<RenderSliver> {
  /// Create a sliver that absorbs overlap and reports it to a
  /// [SwiftSliverOverlapAbsorberHandle].
  ///
  /// The [sliver] must be a [RenderSliver].
  RenderSwiftSliverOverlapAbsorber({
    required SwiftSliverOverlapAbsorberHandle handle,
    RenderSliver? sliver,
  }) : _handle = handle {
    child = sliver;
  }
  ...
  ///
  /// A particular [SwiftSliverOverlapAbsorberHandle] can only be assigned to a
  /// single [RenderSwiftSliverOverlapAbsorber] at a time.
  SwiftSliverOverlapAbsorberHandle get handle => _handle;
  SwiftSliverOverlapAbsorberHandle _handle;
  set handle(SwiftSliverOverlapAbsorberHandle value) {
    if (handle == value) {
      return;
    }
    if (attached) {
  ...
  double _findMaxScrollObstructionExtent(RenderSliver sliver) {
    if (sliver.geometry != null &&
        sliver.geometry!.maxScrollObstructionExtent > 0) {
      return sliver.geometry!.maxScrollObstructionExtent;
    }
    double childObstruction = 0.0;
    sliver.visitChildren((RenderObject child) {
      if (child is RenderSliver) {
        childObstruction += _findMaxScrollObstructionExtent(child);
      }
    });
    return childObstruction;
  }
  ...
  void performLayout() {
    assert(
      handle._writers == 1,
      'A SwiftSliverOverlapAbsorberHandle cannot be passed to multiple RenderSwiftSliverOverlapAbsorber objects at the same time.',
    );
    if (child == null) {
    ...
    final double maxScrollObstructionExtent = _findMaxScrollObstructionExtent(
      child!,
    );
    geometry = childLayoutGeometry.copyWith(
      scrollExtent:
          childLayoutGeometry.scrollExtent - maxScrollObstructionExtent,
      layoutExtent: math.max(
        0.0,
        childLayoutGeometry.paintExtent - maxScrollObstructionExtent,
      ),
    );
    handle._setExtents(maxScrollObstructionExtent, maxScrollObstructionExtent);
  }
...
/// [SwiftSliverOverlapAbsorberHandle].
///
/// The [SwiftSliverOverlapAbsorber] must be an earlier descendant of a common
/// ancestor [Viewport], so that it will always be laid out before the
/// [SwiftSliverOverlapInjector] during a particular frame.
///
/// See also:
///
///  * [SwiftNestedScrollView], which uses a [SwiftSliverOverlapAbsorber] to align its
///    children, and which shows sample usage for this class.
class SwiftSliverOverlapInjector extends SingleChildRenderObjectWidget {
  /// Creates a sliver that is as tall as the value of the given [handle]'s
  /// layout extent.
  const SwiftSliverOverlapInjector({
    super.key,
    required this.handle,
    Widget? sliver,
  }) : super(child: sliver);

  /// The handle to the [SwiftSliverOverlapAbsorber] that is feeding this injector.
  ///
  /// This should be a handle owned by a [SwiftSliverOverlapAbsorber] and a
  /// [SwiftNestedScrollViewViewport].
  final SwiftSliverOverlapAbsorberHandle handle;

  @override
  RenderSwiftSliverOverlapInjector createRenderObject(BuildContext context) {
    return RenderSwiftSliverOverlapInjector(handle: handle);
  }

  @override
  void updateRenderObject(
    BuildContext context,
    RenderSwiftSliverOverlapInjector renderObject,
  ) {
    renderObject.handle = handle;
  }
...
/// [SwiftSliverOverlapAbsorberHandle].
///
/// The [RenderSwiftSliverOverlapAbsorber] must be an earlier descendant of a common
/// ancestor [RenderViewport] (probably a [RenderSwiftNestedScrollViewViewport]), so
/// that it will always be laid out before the [RenderSwiftSliverOverlapInjector]
/// during a particular frame.
class RenderSwiftSliverOverlapInjector extends RenderSliver {
  ...
  /// object.
  ///
  /// This should be a handle owned by a [RenderSwiftSliverOverlapAbsorber] and a
  /// [RenderSwiftNestedScrollViewViewport].
  SwiftSliverOverlapAbsorberHandle get handle => _handle;
  SwiftSliverOverlapAbsorberHandle _handle;
  set handle(SwiftSliverOverlapAbsorberHandle value) {
    if (handle == value) {
  ...
  void performLayout() {
    ...
    assert(
      ...
      'SwiftSliverOverlapInjector has found no absorbed extent to inject.\n '
      'The SwiftSliverOverlapAbsorber must be an earlier descendant of a common '
      'ancestor Viewport, so that it will always be laid out before the '
      'SwiftSliverOverlapInjector during a particular frame.\n '
      'The SwiftSliverOverlapAbsorber is typically contained in the list of slivers '
      'provided by SwiftNestedScrollView.headerSliverBuilder.\n',
    );
...
/// The [Viewport] variant used by [SwiftNestedScrollView].
///
/// This viewport takes a [SwiftSliverOverlapAbsorberHandle] and notifies it any time
/// the viewport needs to recompute its layout (e.g. when it is scrolled).
class SwiftNestedScrollViewViewport extends Viewport {
  /// Creates a variant of [Viewport] that has a [SwiftSliverOverlapAbsorberHandle].
  SwiftNestedScrollViewViewport({
    super.key,
    super.axisDirection,
  ...
  @override
  RenderSwiftNestedScrollViewViewport createRenderObject(BuildContext context) {
    return RenderSwiftNestedScrollViewViewport(
      axisDirection: axisDirection,
      crossAxisDirection:
          crossAxisDirection ??
  ...
  void updateRenderObject(
    BuildContext context,
    RenderSwiftNestedScrollViewViewport renderObject,
  ) {
    renderObject
      ..axisDirection = axisDirection
      ..crossAxisDirection =
...
/// The [RenderViewport] variant used by [SwiftNestedScrollView].
///
/// This viewport takes a [SwiftSliverOverlapAbsorberHandle] and notifies it any time
/// the viewport needs to recompute its layout (e.g. when it is scrolled).
class RenderSwiftNestedScrollViewViewport extends RenderViewport {
